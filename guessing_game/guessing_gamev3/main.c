...
int main() {
  /*
  example of malloc
  we are asking the comuter ti allocate memory of the size of Ranking times 3, the highscore i spointing the first memory blovk allocated by the malloc commad, last step is to set what we coded into memeory
  Ranking* high_scores = malloc(sizeof(Rankinig) * 3)
  memset(high_score, 0, sizeof(Ranking) * 3)
  */

  //we setting that  our that is pointing to nothing
  ...
  */
  ...
  //we are freeing the heap of the memory
  free(high_score);
  return 0;
}